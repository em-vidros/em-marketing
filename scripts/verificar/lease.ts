...
for (const golpe of [
  () => concluir(db, { tarefaId: a.id, epoca: 1 }, { versoes: [] }),
...
concluir(db, { tarefaId: b.id, epoca: 2 }, { versoes: [], resumo: "3 direções" });
...
// falha permanente vai direto para revisão manual
db.query("UPDATE tasks SET estado = 'pendente', tentativas = 1 WHERE id = ?").run(d.id);
const e2 = reivindicar(db, "designer", "wE")!;
falhar(db, { tarefaId: e2.id, epoca: e2.leaseEpoca }, { tipo: "permanente", mensagem: "schema inválido" });
