...
- [ ] **F5 Produção** — código ✅ · pipeline de build ✅ (ghcr publica; watchtower agora responde) · gate ⏳ falta secret `WATCHTOWER_TOKEN`, env de produção, primeiro `compose up`, DNS + Caddy. Dockerfile, GH Actions → ghcr → watchtower, Caddy + DNS, cron do calendário. Gate: US-7; `/health` pelo subdomínio.
