...
- **DNS.** Em 2026-09-15 `mkt.emvidros.com.br` já resolve para 170.247.31.241, o IP de entrada. Até 2026-08-28 apontava para 177.54.129.7, o IP de saída, e o Let's Encrypt falhava. Falta confirmar de fora do servidor que o certificado saiu e o `/health` responde por HTTPS. De dentro do servidor o `curl` para no handshake, o que não prova nada nos dois sentidos. O vhost em `/etc/caddy/sites/mkt.emvidros.com.br.caddy` e a linha em `/etc/caddy/allowed-hosts` já existem. Com o HTTPS no ar, o `setWebhook` destrava.
