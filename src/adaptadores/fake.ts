...
export class FakeRedator implements Redator {
  async legenda(e: {
    ...
    const semente = `${e.pedido.theme}\0${e.direcao.direction_id}\0${e.marca.brandVersionId}\0${e.ajuste ?? ""}`;
    const escolha = fracao(semente, LEGENDAS.length);
    // O ajuste entra no corpo do texto: legenda reescrita que devolvesse os mesmos
    // bytes viraria a mesma versão na deduplicação por conteúdo, e o Ricardo veria
    // o pedido dele sumir sem aviso.
    const reescrita = e.ajuste
      ? `${LEGENDAS[escolha]!(e.pedido.theme)}\n\nAjuste pedido nesta versão: ${e.ajuste}`
      : LEGENDAS[escolha]!(e.pedido.theme);
    return `${reescrita}\n\n${HASHTAGS}`;
  }
